> **This is a customized fork of [Meetily](https://github.com/Zackriya-Solutions/meeting-minutes)** with added MCP server integration and local automation for research lab use. See [What We Added](#what-we-added) below.

---

## What We Added

This fork extends Meetily with a **Claude MCP integration** that enables natural language querying across all historical meetings, and a **one-click startup script** for macOS audio routing.

### MCP Server (`mcp_server/server.py`)

A [Model Context Protocol](https://modelcontextprotocol.io/) server that connects Claude Desktop directly to Meetily's local SQLite database. Claude can query meeting history without any cloud dependency.
...
| `get_meeting_summary` | Get structured summary; auto-falls back to raw transcript when Groq summary is unavailable |
...
### One-Click Startup (`start_meeting.sh`)
...
Currently, the system answers questions you remember to ask. The next phase adds a **Proactive Agent** that automatically runs after each meeting to:
- Detect overdue action items (cross-referenced with meeting history via semantic vector search)
- Surface conflicting decisions across meetings
- Push findings without requiring manual queries

This will use a LangGraph StateGraph pipeline with hybrid BM25+FAISS retrieval, replacing the current keyword-only SQL search with semantic understanding.
